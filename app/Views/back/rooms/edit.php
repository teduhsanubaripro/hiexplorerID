      ...
      <div class="card">
        <div class="card-body col-md-6">
          <?php $validation =  \Config\Services::validation(); ?>
            <form action="<?=site_url('rooms/update/'.$rooms->room_id)?>" method="post" autocomplete="off" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <div class="form-group">
                    ...
                    <select name="hotel_id" class="form-control <?= $validation->hasError('hotel_id') ? 'is-invalid' : '' ?>">
                        ...
                        <?php foreach ($hotels as $hotel): ?>
                            <option value="<?= $hotel->hotel_id ?>" <?= old('hotel_id', $rooms->hotel_id) == $hotel->hotel_id ? 'selected' : '' ?>>
                                <?= $hotel->name ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="room_type" value="<?=old('room_type', $rooms->room_type)?>" class="form-control <?=$validation->hasError('room_type') ? 'is-invalid' : null?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="price_per_night" value="<?=old('price_per_night', $rooms->price_per_night)?>" class="form-control <?=$validation->hasError('price_per_night') ? 'is-invalid' : null?>">
                </div>
                <div class="form-group">
                    <label>Description</label>
                    <textarea style="height: 200px; width: 100%;" name="description" class="form-control"><?=$rooms->description?></textarea>
                </div>
                <div class="form-group">
                    <label>Status?</label>
                ...
                <div class="form-group">
                    ...
                    <?php if (!empty($rooms->image_url)): ?>
                        <?php $images = explode(',', $rooms->image_url); ?>
                        <div>
                            <?php foreach ($images as $image): ?>
                                <img src="<?= base_url('uploads/rooms/' . trim($image)) ?>" alt="Car Image" width="100" class="mb-2">
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
                </div>
